    ...
    <React.Fragment>
        <a href="https://nodei.co/npm/quantlib-wasm" rel="nofollow">
            <img
                ...
                alt="NPM"
                datacanonicalsrc="https://nodei.co/npm/quantlib-wasm.png?downloads=true&amp;downloadRank=true"
                style={{ maxWidth: "100%" }}
            />
        </a>
...
class App extends React.Component {
    constructor(props) {
        ...
        this.setPlotlyRef = (element) => {
            this.plotlyElement = element;
            const { QuantLib, marketData } = this.state;
            if (!QuantLib || !marketData) {
                return;
            }
            ...
            let layout = {
                yaxis: { color: "#ffb15e", gridcolor: "#454d55", tickformat: ".2%f", zeroline: false },
                xaxis: { color: "#ffb15e", gridcolor: "#454d55", hoverformat: "%Y-%m-%d" },
                paper_bgcolor: "#222",
                plot_bgcolor: "#343434",
            ...
            Plotly.newPlot(element, data, layout, { showSendToCloud: true });
            // console.log(element);
        };
        ...
        QuantLib.onRuntimeInitialized = () => {
            ...
            this.setState({ status: "Done!", QuantLib, marketData });
        };
        this.state = { status: "Loading...", QuantLib: null };
    }

    render() {
        const { status, QuantLib, marketData } = this.state;
        if (marketData) {
            marketData.forEach((d) => {
                if (!("quote" in d)) {
                    Object.defineProperty(d, "quote", { get: () => `${(100 * d.price).toFixed(4)}%` });
                }
            });
        }
        var headers = ["Instrument", "Type", "Price"];
        var columns = ["name", "type", "quote"];
        return (
            <div className="container">
                <div className="row">
                    <div className="col-12">
                        ...
                        <p className="text">
                            This app uses QuantLib WebAssembly, which is a JavaScript compilation of the quantitative finance library QuantLib (codebase in
                            C++). The app fetches latest market prices and bootstraps the interest yield curve in pure JavaScript. No server-side calculations
                            or rendering is needed. The app only loads static content.
                        </p>
                    </div>
                </div>
                <div className="row">
                    <div className="col-12 col-md-6">{marketData ? <Table headers={headers} columns={columns} rows={marketData} /> : <p>{status}</p>}</div>
                    <div className="col-12 col-md-6">{marketData && QuantLib ? <div id="chart" ref={this.setPlotlyRef}></div> : null}</div>
                </div>
                <div className="row">
                    <div className="col-12">
                        <Footer />
